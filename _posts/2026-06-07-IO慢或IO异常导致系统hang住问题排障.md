---
title: IO慢或IO异常导致系统hang住问题排障
date: 2026-06-07 22:07:00 +0800
categories: [linux,系统运维,IO慢导致系统hang]
tags: [linux,系统运维,IO慢导致系统hang]
---
...
翻译过来就是：一般情况下，linux会把可用内存的40%的空间作为文件系统的缓存。当到达这个阈值后，文件系统将缓存中的数据整体写入到磁盘中， 导致后续的IO请求均同步。但是系统对写入时间有最大120秒的限制。如果文件系统不能在时间限制之内完成数据同步，则会发生上述的错误。这通常发生在内存很大的系统上。系统内存大，则缓冲区大，同步数据所需要的时间就越长，超时的概率就越大。
或者IO子系统的处理不够快速，没有IO数据产生的快，导致越来越多的请求堆积，导致系统失去响应。

下面说一下内核hung task检测机制由来。我们知道进程等待IO时，经常处于D状态，即TASK_UNINTERRUPTIBLE状态，处于这种状态的进程不处理信号，所以kill不掉，如果进程长期处于D状态，那么肯定不正常，原因可能有二：

1. IO路径上的硬件出问题了，比如硬盘坏了或者使用nfs，和nfs网络异常断开
2. IO处理缓慢，没有IO数据产生的快
3. 内核自己出问题了
...
### 三、解决办法
#### 1、检测hung_task_panic是否开启
当进程长时间处于 D 状态（不可中断睡眠状态，通常因 I/O 等待或死锁引起）时，内核会判定该任务为 Hung Task。
kernel.hung_task_panic  当内核检测到 Hung Task 时，是否触发内核 Panic
这个一般都是关闭的，即使hung_task_panic是关闭状态，也有可能由于进程长时间hung导致系统异常

##### 2）临时关闭内核参数
echo 0 > /proc/sys/kernel/hung_task_panic
##### 3）永久修改
vi /etc/sysctl.conf打开文件添加
...
#### 3、取消120秒时间限制(不一定有用,保持120s默认即可)
kernel.hung_task_timeout_secs 定义任务在 D 状态停留多长时间才会被判定为 Hung Task。默认值为 120 秒
如果kernel.hung_task_timeout_secs的值设置为0，就是取消进程被判断为hung，不会有进程被判断为hung
